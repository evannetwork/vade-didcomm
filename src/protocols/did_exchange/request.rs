...
use crate::{BaseMessage, MessageWithBody, StepResult, get_step_output_decrypted, protocol::DID_EXCHANGE_PROTOCOL_URL, utils::SyncResult, write_db};
...
pub fn receive_request(message: &str) -> StepResult {
    ...
    let to_vec = parsed_message.to.as_ref().ok_or("to is required")?;
    let to_did = &to_vec[0];
    let thread_id = parsed_message.other.get("thid").ok_or("thid is required")?;
