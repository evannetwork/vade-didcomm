use crate::{
    datatypes::{PresentProofReq, MessageWithBody, BaseMessage, PresentationData, ExtendedMessage},
...
pub fn send_presentation(message: &str) -> StepResult {
    ...
    let presentation_request = saved_presentation_data.presentation_attach.ok_or("Presentation data not attached.")?;
...
pub fn receive_request_presentation(message: &str) -> StepResult {
    ...
    let presentation_request = saved_presentation_data.presentation_attach.ok_or("Presentation request not attached.")?;
    let metadata = presentation_request.get(0).ok_or("Request data not attached")?;
...
pub fn send_propose_presentation(message: &str) -> StepResult {
    ...
    let saved_presentation_data = get_presentation(&exchange_info.from,&exchange_info.to,)?;
    let presentation_request = saved_presentation_data.presentation_proposal.ok_or("Presentation data not attached.")?;
 
    return generate_step_output(&serde_json::to_string(&request_message)?, &serde_json::to_string(&presentation_request)?);
}