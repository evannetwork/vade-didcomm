use crate::{datatypes::{BaseMessage, MessageWithBody, ExtendedMessage,PresentationData}, get_from_to_from_message, presentation::{save_presentation,get_presentation}, protocols::protocol::{generate_step_output, StepResult}};
...
pub fn send_request_presentation(message: &str) -> StepResult {
    ...
    let data = &serde_json::to_string(&parsed_message.body.ok_or("Presentation data not provided.")?)?;
    ...
    let presentation_request = presentation_data.presentation_attach.ok_or("Presentation request not attached.")?;
    let metadata = presentation_request.get(0).ok_or("Request data not attached")?;
    return generate_step_output(&serde_json::to_string(&request_message)?, &serde_json::to_string(metadata)?);
}
...
pub fn receive_presentation(message: &str) -> StepResult {
    ...
    let base_message:BaseMessage = BaseMessage{
        ...
        to: Some(parsed_message.to.clone().ok_or("To DID not provided")?.to_vec()),
    };
    let exchange_info = get_present_proof_info_from_message(parsed_message)?;
    let base_info = get_from_to_from_message(base_message)?; 

    let presentation_data = exchange_info.presentation_data.ok_or("Presentation data not provided.")?;
    ...
    let presentation = saved_presentation_data.presentation_attach.ok_or("Presentation request not attached.")?;
...
pub fn receive_propose_presentation(message: &str) -> StepResult {
    ...
    let saved_presentation_data = get_presentation(&base_info.from,&base_info.to,)?;
    let presentation_proposal = saved_presentation_data.presentation_proposal.ok_or("Presentation request not attached.")?;
    return generate_step_output(message, &serde_json::to_string(&presentation_proposal)?);
}