...
pub fn get_keypair_set() -> KeyPairSet {
    ...
    let sender_options: DidCommOptions = DidCommOptions {
        key_information: Some(KeyInformation::SecretPublic {
            my_secret: alice_secret_key.to_bytes(),
            others_public: bob_public.to_bytes(),
        }),
        sign_key: Some(sign_keypair.secret.to_bytes()),
    };
    let sender_options_stringified = serde_json::to_string(&sender_options).unwrap_or_else(|_| "{}".to_string());

    let receiver_options: DidCommOptions = DidCommOptions {
        key_information: Some(KeyInformation::SecretPublic {
            my_secret: bob_secret_key.to_bytes(),
            others_public: alice_public.to_bytes(),
        }),
        sign_key: Some(sign_keypair2.secret.to_bytes()),
    };
    let receiver_options_stringified = serde_json::to_string(&receiver_options).unwrap_or_else(|_| "{}".to_string());
